...
#define sceKernelDcacheWritebackAll(x) (void)0
...
int intraFontGetGlyph(unsigned char *data, unsigned long *b, unsigned char glyphtype, signed long *advancemap, Glyph *glyph)
{
	...
	if (glyph->flags & PGF_CHARGLYPH) {
		...
		(*b) += 24 + ((glyph->flags & PGF_NO_EXTRA1)?0:56) +
			...
			((glyph->flags & PGF_NO_EXTRA3)?0:56); //skip to 6bits before end of header
...
static int intraFontTile(intraFont *font)
{
...
intraFont* intraFontLoad(const char *filename, unsigned int options)
{
	...
	static const unsigned char bw_shadow[] = {
		0x10, 0x11, 0x11, 0x01, 0x10, 0x22, 0x22, 0x01, 0x21, 0x43, 0x34, 0x12, 0x31, 0x75, 0x57, 0x13,
...
void intraFontSetStyle(intraFont *font, float size, unsigned int color, unsigned int shadowColor, float angle, unsigned int options)
{
	...
	if (font->angle != angle) { // Avoid recomputations
		...
		if (angle == 0.f) {
			...
			font->Rsin = sinf(angle*M_PI/180.f);
			font->Rcos = cosf(angle*M_PI/180.f);
		}
...
float intraFontPrintColumnEx(intraFont *font, float x, float y, float column, const char *text, int length)
{
	...
	if (column >= 0) {
		...
		//(hack to share local buffer between intraFontPrint and intraFontMeasure)																				//negative column prevents from drawing -> measure text
		x = intraFontMeasureTextUCS2Ex(font, ucs2_text, length);
	}
...
float intraFontPrintColumnUCS2Ex(intraFont *font, float x, float y, float column, const cccUCS2 *text, int length)
{
	...
	fontVertex *v, *v0, *v1, *v2, *v3, *s0, *s1, *s2, *s3;
	...
	v = sf2d_pool_malloc(4 * (n_glyphs+n_sglyphs) * sizeof(fontVertex));
					...
					if (textwidth > column) {	 //scrolling really required
						...
						switch (font->options & PGF_SCROLL_MASK) {
							...
							case INTRAFONT_SCROLL_LEFT:																	 //scroll left
								sf2d_set_scissor_test(GPU_SCISSOR_NORMAL, left-2, 0, left+column+4, 240); //limit to column width
								if (count < 60) {
							...
							case INTRAFONT_SCROLL_SEESAW:	 //scroll left and right
								sf2d_set_scissor_test(GPU_SCISSOR_NORMAL, left-column/2-2, 0, left+column/2+4, 240); //limit to column width
								textwidth -= column;
								if (count < 60) {
							...
							case INTRAFONT_SCROLL_RIGHT:	//scroll right
								sf2d_set_scissor_test(GPU_SCISSOR_NORMAL, left-column-2, 0, left+4, 240); //limit to column width
								if (count < 60) {
		...
		if (char_id < font->n_chars) {
			...
			for (j = 0; j < 3; j++) {
				...
				if (glyph_id < font->n_chars) {
					...
					// Tex coords
					ul = (glyph->x - 0.25f)                 / (float)font->texWidth;
					ur = (glyph->x + glyph->width + 0.25f)  / (float)font->texWidth;
			...
			if (c_index > last_c_index) {
				...
				// Tex coords
				ul = (shadowGlyph->x - 0.25f)                       / (float)font->texWidth;
				ur = (shadowGlyph->x + shadowGlyph->width + 0.25f)  / (float)font->texWidth;
	...
	if (font->fileType == FILETYPE_BWFON) {
		...
		for (i = 0; i < n_glyphs; i++) {
			GPU_SetAttributeBuffers(
				2, // number of attributes
				(u32*)osConvertVirtToPhys((u32)(v + n_sglyphs*4 + i*4)),
				GPU_ATTRIBFMT(0, 3, GPU_FLOAT) | GPU_ATTRIBFMT(1, 2, GPU_FLOAT),
				0xFFFC, //0b1100
				0x10,
				1, //number of buffers
...
{
	//explanation: intraFontPrintColumnEx does the String -> UCS2 conversation,
	return intraFontPrintColumnEx(font, 0.f, 0.f, -1.0f, text, length);																							//but a negative column width triggers measurement without drawing
}
