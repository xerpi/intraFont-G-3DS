...
int main()
{
	...
	// Colors
	enum colors {
		RED      = RGBA8(0xFF, 0x00, 0x00, 0xFF),
		GREEN    = RGBA8(0x00, 0xFF, 0x00, 0xFF),
		BLUE     = RGBA8(0x00, 0x00, 0xFF, 0xFF),
		WHITE    = RGBA8(0xFF, 0xFF, 0xFF, 0xFF),
		LITEGRAY = RGBA8(0xBF, 0xBF, 0xBF, 0xFF),
		GRAY     = RGBA8(0x7F, 0x7F, 0x7F, 0xFF),
		DARKGRAY = RGBA8(0x3F, 0x3F, 0x3F, 0xFF),
		BLACK    = RGBA8(0x00, 0x00, 0x00, 0xFF),
	};
	...
	int i;
	for (i = 0; i < 16; i++) {
		sprintf(file,"/font/ltn%d.pgf",i);
		ltn[i] = intraFontLoad(file, 0);                   //<- this is where the actual loading happens
		intraFontSetStyle(ltn[i], 1.0f, WHITE, DARKGRAY, 0.0f, 0);
		printf("%d%%\n", (i+1)*100/20);
	}

	intraFont* jpn0 = intraFontLoad("/font/jpn0.pgf",INTRAFONT_STRING_SJIS); //japanese font with SJIS text string encoding
	intraFontSetStyle(jpn0,0.8f,WHITE,DARKGRAY,0.f,0);                       //scale to 80%
	printf("%d%%\n",17*100/20);

	intraFont* kr0 = intraFontLoad("/font/kr0.pgf",INTRAFONT_STRING_UTF8);  //Korean font (not available on all systems) with UTF-8 encoding
	intraFontSetStyle(kr0,0.8f,WHITE,DARKGRAY,0.f,0);                       //scale to 80%
	printf("%d%%\n",18*100/20);

	intraFont* arib = intraFontLoad("/font/arib.pgf",0);  //Symbols (not available on all systems)
	intraFontSetStyle(arib,0.8f,WHITE,DARKGRAY,0.f,0);    //scale to 80%
	printf("%d%%\n",19*100/20);

	intraFont* chn = intraFontLoad("/font/gb3s1518.bwfon",0); //chinese font
	intraFontSetStyle(chn,0.8f,WHITE,DARKGRAY,0.f,0);         //scale to 80%
	printf("100%%\ndone\n");

	// Make sure the important fonts for this application are loaded
	if(!ltn[0] || !ltn[4] || !ltn[8]) return -1;
